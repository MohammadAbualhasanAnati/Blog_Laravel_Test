            ...
            <div class="card-body">
                ...
                <div class="footer">
                    @if(Auth::user()->type=="admin")
                        <label class="p-1 mb-2 text-light {{$post->published?'bg-success':'bg-dark'}}">{{$post->published?"published":"not published"}}</label>
                        <div class="buttons-container">
                            <form class="form-inline" method="POST" action="/posts/publish/{{$post->id}}">
                                @csrf
                                <button class='btn {{$post->published?"btn-danger":"btn-success"}}'>{{$post->published?"unpublish":"publish"}}</button>
                            </form>
                            <form class="form-inline" method="GET" action="/posts/edit/{{$post->id}}">
                                @csrf
                                <button class='btn btn-secondary'>Edit</button>
                            </form>
                            <form class="deleteForm" class="form-inline" method="POST" action="/posts/delete/{{$post->id}}">
                                @csrf
